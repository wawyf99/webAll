        ...
        Autocomplete.prototype = {
            ...
            select: function() {
                ...
                if (typeof this.callback == "function") {
                    this.callback(this.$menu.find('.active'));
                }
                return this.hide()
            }
            ...
            render: function(items) {
                var that = this,
                    _href = that.options.href ? that.options.href : false;

                items = $(items).map(function(i, item) {
                    var _newLink = _href ? _href + item.id : 'javascript:;';
                    i = $(that.options.item).attr(that.setValue(item))
                    i.find('a').attr("href",_newLink).html(that.highlighter(item))
                    return i[0]
                })
...
/*            ,
        ...
        $.fn.autocomplete.defaults = {
            source: function(query, process) {
                if (query.indexOf("@") == -1) {
                    return [query + "@yahoo.com.hk", query + "@hotmail.com", query + "@gmail.com", query + "@pchome.com.hk", query + "@yahoo.com", query + "@kimo.com", query + "@kimo.com.hk"]
                }
            },
            items: 8,
